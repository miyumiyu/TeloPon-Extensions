...
_L = {
    "ja": {
        ...
        "prompt_cmd_auth": (
            ...
            "* 「今どんな感じ？」「途中経過は？」「アンケートの状況は？」→ [CMD]NC:poll status\n"
        ...
        "poll_status_cue": (
            "【番組ディレクターからのカンペ】\n"
            "アンケート途中経過です。質問:「{question}」(状態: {status})\n{results}\n"
            "この途中経過を配信者に報告してください。"
        ),
        "poll_status_none": (
            "【番組ディレクターからのカンペ】\n"
    ...
    "en": {
        ...
        "prompt_cmd_auth": (
            ...
            "* Poll progress: [CMD]NC:poll status\n"
        ...
        "poll_status_cue": (
            "[Cue from Director]\n"
            "Poll progress: \"{question}\" (status: {status})\n{results}\n"
            "Report this to the streamer."
        ),
        "poll_status_none": (
            "[Cue from Director]\n"
...
class NiconicoLivePlugin(BasePlugin):
    ...
    def _fetch_info_thread(self):
        ...
        try:
            ...
            # 配信情報をコア共有機構にセット
            nico_url = f"https://live.nicovideo.jp/watch/{self._nicolive_id}"
            self.set_stream_info(
                url=nico_url, thumbnail=self._thumbnail_bytes,
                title=self._title, platform="Niconico",
            )
    ...
    def get_prompt_addon(self) -> str:
        ...
        parts.append(self.get_thumbnail_prompt_addon())
        return "".join(parts)
    ...
    def start(self, prompt_config, plugin_queue):
        ...
        if not _HAS_DEPS:
            logger.warning(f"{_TAG} 依存ライブラリ不足のため起動をスキップします")
            return
    ...
    def _on_enquete(self, enquete):
        # 常にキャッシュ（途中経過含む）
        self._last_enquete = enquete

        if not self.plugin_queue:
            return
        # 結果表示 or 終了時のみ自動通知
        if enquete.status in ("Result", "Closed") and enquete.results_per_mille:
            cue = _t("enquete_result_cue",
                      question=enquete.question,
                      results=self._format_enquete_results(enquete))
            self.send_text(self.plugin_queue, cue)
            logger.info(f"{_TAG} 📊 アンケート結果: {enquete.question}")

    @staticmethod
    def _format_enquete_results(enquete) -> str:
        lines = []
    ...
    def handle(self, value: str):
        ...
        if v_lower.startswith("viewers"):
            self._cmd_viewers()
            return
        if v_lower.startswith("poll") and v_lower.split(None, 1)[-1].startswith("status"):
            self._poll_status()
            return
    ...
    def _poll_status(self):
        """アンケートの途中経過をAIに通知する（ログイン不要・NDGRキャッシュから取得）"""
        if not self.plugin_queue:
            return
        eq = self._last_enquete
        if not eq or not eq.question:
            cue = _t("poll_status_none")
        elif eq.status == "Closed":
            cue = _t("poll_status_closed")
        else:
            cue = _t("poll_status_cue",
                      question=eq.question,
                      status=eq.status,
                      results=self._format_enquete_results(eq))
        self.send_text(self.plugin_queue, cue)
