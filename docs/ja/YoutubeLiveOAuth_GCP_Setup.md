...
## 事前準備: 2段階認証プロセス（MFA）の有効化

Google Cloud Console にアクセスするには、Googleアカウントの **2段階認証プロセス（MFA）** が有効になっている必要があります。  
未設定の場合は以下の画面が表示されます。「設定に移動」から有効にしてください。

<img src="../images/YoutubeLiveOAuth/YoutubeLiveOAuth2FAError.png" width="500">

> 2段階認証を有効にした後、Google Cloud Console にアクセスできるようになるまで60秒ほどかかる場合があります。
...
プロジェクト作成後、左メニューから「Google Auth Platform」を開きます。  
...
左メニューの「クライアント」をクリックし、認証情報画面で「OAuthクライアントIDを作成」をクリックします。
